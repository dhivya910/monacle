...
export type EventType = {
   ...
   * like `https://monacal.com/[user-slug]/[event-type-slug]`.
   */
  slug?: string;
};
...
export async function createUserByEmail(email: string): Promise<User> {
  ...
  if (existingUser) {
    throw new Error(`The email is already used by another user: ${email}`);
  }
  const newUser = {
    id: crypto.randomUUID(),
    email,
...
/** Returns true if the user's settings are ready to start using Ofc Hrs.
