    ...
    <Layout pageTitle="All Events">
      <div className="bg-gray-900">
        <div className="mx-auto max-w-7xl px-6 text-center lg:px-8">
          ...
          <h1 className="text-xl tracking-tight text-white pt-20">
            Happening in Monaverse...{" "}
          </h1>
          <ul
            role="list"
            ...
            {tab == "allEvents"
              ? events.map((event: any) => (
                  <li key={event.title} className=" bg-gray-800 px-8 py-10">
                    ...
                    <p className="text-sm leading-6 text-gray-400 truncate ...">
                      {event.description}
                    </p>
                    <ul
                      role="list"
                    ...
                    >
                      <div className="flex">
                        <svg
                          className="h-5 w-5 mx-1 text-white"
                          width="24"
                          height="24"
                          viewBox="0 0 24 24"
                          stroke-width="2"
                      ...
                      <div className="flex mx-2">
                        ...
                        <p className="text-sm">
                          {new Date(event.startTime).toString().split(" ")[4]}
                        </p>
                      </div>
                    </ul>
                    {new Date(event.startTime) > new Date() ? (
                      <>
                        <button
                          type="button"
                          className="text-gray-900 bg-gray-100 font-medium rounded-full text-sm mt-4 px-5 py-2.5 me-2 mb-2 dark:bg-gray-800 dark:hover:bg-gray-700 dark:focus:ring-gray-700 dark:border-gray-700"
                          onClick={() => setisSuccess(true)}
                        >
                          Register Now
                        </button>
                        {!!address ? <Success/>:<Alert/>}                   </>
                    ) : (
                      <p className="bg-gray-700 text-sm my-4 py-1">
                        {" "}
